...
namespace Saucy.Modules.XP {
  public abstract class DataXPReceive : ScriptableObject {
    ...
    [SerializeField] protected DataXPFormula selectedXPFormula;
    ...
    public virtual void Init (XPReceiver _caller) {
      // Sets the reference of the caller (XPReceiver) so we can use it's UnityEvents for updating values.
      caller = _caller;
    }
